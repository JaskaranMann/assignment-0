...
TEST_CASE("Default constructor - 1 point") {
    ...
    Box myBox;
    if (myBox.getWidth() != 0) {
        INFO("myBox.getWidth() returned " << myBox.getWidth());
        INFO("Expected 0");
        FAIL();
    }

    if (myBox.getHeight() != 0) {
        INFO("myBox.getHeight() returned " << myBox.getHeight());
        INFO("Expected 0");
        FAIL();
    }

    if (myBox.getLength() != 0) {
        INFO("myBox.getLength() returned " << myBox.getLength());
        INFO("Expected 0");
        FAIL();
    }
}
